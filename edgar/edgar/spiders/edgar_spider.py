...
class edgarSpiderSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.sec.gov/Archives/edgar/data/1065280/000119312508040378/d10k.htm',
    ...
    def parse_document(self, response):
        ...
        year = response.xpath("//text()[re:test(., '\d{2}\, \d{4}')]")[0].extract().split(',')[-1].strip()
        year = re.sub('\D','',year)
        ...
        for i in range(len(sections)-1):
            ...
            ## Define start and ending paths. The problem is that many documents have different CSS formats...
                start = "(preceding::font[re:test(text(),'ITEM\s" + sections[i] + "\. ')] | preceding::b[re:test(text(),'Item\s" + sections[i] + "\.')] | preceding::font[contains(@style,'font-weight:bold') and re:test(text(),'Item\s" + sections[i] + "\.')] | preceding::b[re:test(.,'Item\s" + sections[i] + "\.')])"

                end = "(following::font[re:test(text(),'ITEM\s" + sections[i+1] + "\. ')] | following::b[re:test(text(),'Item\s" + sections[i+1] + "\.')] | following::font[contains(@style,'font-weight:bold') and re:test(text(),'Item\s" + sections[i+1] + "\.')]  | preceding::b[re:test(.,'Item\s" + sections[i+1] + "\.')])"

                content = response.xpath("//text()[string-length() > 75 and " + start  + " and " + end + "]").extract()
